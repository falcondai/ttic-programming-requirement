...
def train(env_spec, env_step, env_reset, env_render, args, build_model):
    ...
    with tf.Graph().as_default() as g:
        ...
        entropy_reg = - tf.reduce_mean(tf.reduce_sum(action_probs \
        ...
        if not args['no_summary']:
            ...
            for g, v in pg_grad_vars:
                tf.histogram_summary('pg_gradients/%s' % v.name, g)
                print 'pg_gradients/%s' % v.name
            for g, v in v_grad_vars:
                tf.histogram_summary('v_gradients/%s' % v.name, g)
        ...
        with tf.Session() as sess:
            ...
            for i in tqdm.tqdm(xrange(args['n_train_steps'])):
                ...
                last_done, _last_obs_q = sample_ticks(
                    ...
                    last_done,
                    env_render,
                    n_update_ticks=args['n_update_ticks'],
                ...
                obs = list(duplicate_obs(list(last_obs_q)[:-1] + observations,
                                         args['n_obs_ticks']))
                next_obs = obs[1:] + [np.zeros(policy_input_shape)]
                ...
                for j in xrange(n_batch):
                    ...
                    pg_grad_vars_val, v_grad_vars_val, entropy_reg_val = sess.run([
                        pg_grad_vars,
                        v_grad_vars,
                        entropy_reg,
                        ], feed_dict=grad_feed)
                ...
                update_dict = {
                    ...
                    avg_reg_ph: acc_reg / args['n_update_ticks'],
                }
                update_dict.update(pg_acc_grads)
                update_dict.update(v_acc_grads)
                summary_val, _, _ = sess.run([summary_op,
                                              update_policy_op,
                                              update_v_op],
